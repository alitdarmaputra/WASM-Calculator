...
EXTERN EMSCRIPTEN_KEEPALIVE void appendNumber(int number) {
  ...
  if (number == '.' && strcmp(calculator.currentOperand, "") == 0) {
    strcat(calculator.currentOperand, "0\0");
  }
...
EXTERN EMSCRIPTEN_KEEPALIVE void delete() {
  ...
  if (strlen(calculator.currentOperand) > 0) {
    slice(calculator.currentOperand, buffer, 0,
          strlen(calculator.currentOperand) - 1);
...
EXTERN EMSCRIPTEN_KEEPALIVE void compute() {
  ...
  if (isInt(result)) {
    ...
    sprintf(calculator.currentOperand, "%g", result);
  }
